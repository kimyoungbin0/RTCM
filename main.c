...
int choice1(int gameplayers[][7], int* totaltok, int turn); // 색깔이 서로 다른 3개의 보석 토큰 가져오기 
...
int rule(int gameplayers[][7], int card_discount[][5], int user_have_nob[], int players, int turn, Card_nob* setnob); // 귀족카드 얻기
int check_score(int gameplayers[][7], int turn); // 플레이어의 점수가 15점인지 확인
void find_winner(int gameplayers[][7], int players, int card_discount[][5]); // 1등 찾기
void print_map(int players, int gameplayers[][7], Card* blue, Card* orange, Card* green, Card_nob* nob, int* totaltok, int* emtycard, int card_discount[][5], Card keepcard[][20], int turn); // 세팅 시각화
        ...
        for (int n = 0; n < players; n++)
        {
            ...
            if (check_score(gameplayers, n) == 1)
            {
                ...
                while (n < players)
                {
                    ...
                    rule(gameplayers, card_discount, user_have_nob, players, n, setnob);
                    n++;
                }
                check_break = 1;
                break;
